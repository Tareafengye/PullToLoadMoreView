 ...
 */
public class PullUpGoodsToLoadMore extends ViewGroup {
    public static String TAG = PullUpGoodsToLoadMore.class.getName();

    MyGoodsScrollView topScrollView, bottomScrollView;
    ...
    public PullUpGoodsToLoadMore(Context context) {
        super(context);
        init();
    }

    public PullUpGoodsToLoadMore(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public PullUpGoodsToLoadMore(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        ...
        post(new Runnable() {
            @Override
            public void run() {
                topScrollView = (MyGoodsScrollView) getChildAt(0);
                bottomScrollView = (MyGoodsScrollView) getChildAt(1);
                topScrollView.setScrollListener(new MyGoodsScrollView.ScrollListener() {
                    @Override
                ...
                bottomScrollView.setScrollListener(new MyGoodsScrollView.ScrollListener() {
                    @Override
                    public void onScrollToBottom() {
                    }
                ...
                if (bottomScrollVIewIsInTop) {
                    ...
                    if (dy < 0 && currPosition == 1) {
                        if (Math.abs(dy) >= scaledTouchSlop) {
                            if(PublicGoodsStaticClass.IsTop){//如果viewpager里边的scrollview在最顶部，，就让外边的scrollview获取焦点，否则，让最里边的scrollview获取焦点
                                isIntercept = true;
                            }
                        }
                    }
